...
func (rp RizinPackage) buildMeson(cfg BuildConfig) error {
	srcPath := rp.sourcePath(cfg.ArtifactsDir)
	args := []string{"setup"}
	args = append(args, rp.PackageSource.BuildArguments...)
	args = append(args, fmt.Sprintf("--prefix=%s/.local", xdg.Home))
	if cfg.PkgConfigDir != "" {
		args = append(args, fmt.Sprintf("--pkg-config-path=%s", cfg.PkgConfigDir))
	}
	if cfg.CMakeDir != "" {
		args = append(args, fmt.Sprintf("--cmake-prefix-path=%s", cfg.CMakeDir))
	}
...
func (rp RizinPackage) buildCMake(cfg BuildConfig) error {
	srcPath := rp.sourcePath(cfg.ArtifactsDir)
	args := []string{}
	args = append(args, rp.PackageSource.BuildArguments...)
	args = append(args, fmt.Sprintf("-DCMAKE_INSTALL_PREFIX=%s/.local", xdg.Home))
	if cfg.CMakeDir != "" {
		args = append(args, fmt.Sprintf("-DCMAKE_PREFIX_PATH=%s", cfg.CMakeDir))
	}
	args = append(args, "-B")
	args = append(args, "build")
...
func (rp RizinPackage) installMeson(site BuildConfig) ([]string, error) {
	srcPath := rp.sourcePath(site.ArtifactsDir)
...
func (rp RizinPackage) installCMake(site BuildConfig) ([]string, error) {
	srcPath := rp.sourcePath(site.ArtifactsDir)
...
func (rp RizinPackage) uninstallMeson(site BuildConfig) error {
	srcPath := rp.sourcePath(site.ArtifactsDir)
...
func (rp RizinPackage) Build(site BuildConfig) error {
	if site.PkgConfigDir == "" && site.CMakeDir == "" {
	...
	if rp.isGitRepo() || err != nil || !fi.IsDir() {
		err := rp.Download(site.ArtifactsDir)
		if err != nil {
			return err
		}
	}
...
func (rp RizinPackage) Install(site BuildConfig) ([]string, error) {
	err := rp.Build(site)
	if err != nil {
		return []string{}, err
	}
...
func (rp RizinPackage) Uninstall(site BuildConfig) error {
